...
const moviesData = [
  { title: "빈센조", release_year: 2021, type: "드라마" },
  { title: "이태원 클라쓰", release_year: 2020, type: "드라마" },
  { title: "재벌집 막내아들", release_year: 2022, type: "드라마" },
  { title: "도깨비", release_year: 2016, type: "드라마" },
  { title: "기생충", release_year: 2019, type: "영화" },
  { title: "그해 우리는", release_year: 2021, type: "드라마" },
  { title: "미스터 션샤인", release_year: 2018, type: "드라마" },
  { title: "서울의 봄", release_year: 2023, type: "영화" },
  { title: "오징어 게임", release_year: 2021, type: "드라마" },
  { title: "이상한 변호사 우영우", release_year: 2022, type: "드라마" },
  { title: "킹덤", release_year: 2019, type: "드라마" },
  { title: "어벤져스: 에이지 오브 울트론", release_year: 2015, type: "영화" },
  { title: "괴물", release_year: 2006, type: "영화" },
  { title: "호텔 델루나", release_year: 2019, type: "드라마" },
  { title: "스물다섯 스물하나", release_year: 2022, type: "드라마" },
  { title: "건축학개론", release_year: 2012, type: "영화" },
  { title: "별에서 온 그대", release_year: 2013, type: "드라마" },
  { title: "더 글로리", release_year: 2022, type: "드라마" },
  { title: "눈물의 여왕", release_year: 2024, type: "드라마" }
];
...
async function seed() {
  ...
  // 1. Clean existing records if desired (optional)
  // We can let them append, or clear tables.
  // Clearing is cleaner to prevent duplicates.
  console.log('Clearing scenes, spots, and movies tables...');
  ...
  for (const movie of moviesData) {
    const { data, error } = await supabase
      ...
      .insert({
        ...
        poster_url: '' // empty or default placeholder
      })
      .select()
      .single();
  ...
  console.log('Inserting spots and scenes...');
  for (const spot of spotsData) {
    ...
    if (spotError) {
      console.error(`Error inserting spot ${spot.name}:`, spotError);
      continue;
    }

    const movieId = movieMap.get(spot.movieTitle);
    if (!movieId) {
      console.warn(`Movie ${spot.movieTitle} not found in map, skipping scene creation.`);
      continue;
    }
    ...
    if (sceneError) {
      console.error(`Error inserting scene for ${spot.name}:`, sceneError);
    }
  }
...
seed().catch(err => {
  console.error('Seed process crashed:', err);
});
